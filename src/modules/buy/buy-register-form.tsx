...
import type { BuyRegister, ProductColor, ProductTypeWithSizes } from "@/lib/db/schema";
...
export const buyRegisterForms = z.array(z.object({
...
export function BuyRegisterFormItem({
  form,
  productTypes,
  productColors,
  onChange,
  ...
  return (
    <div className="space-y-2 w-full relative">
      <div className="flex items-center space-x-2 w-full sticky top-0 bg-background z-10 pb-1">
        <Label className="block w-full">
          <span className="mb-1 block">{t("buyRegisters.product")}</span>
        ...
        <Label className="block w-full">
          <span className="mb-1 block">{t("buyRegisters.amount")}</span>
              ...
              <div className="flex-1">
                {sizesBought.includes(s.productSizeId) && (
                  <div className="space-y-2 border rounded p-2">
                    <p className="font-bold">{t("buyRegisters.quantity")}</p>
                    {s.colors?.map((c) => (
                      <Label key={c.productColorId} className="flex items-center w-full">
                        <span className="mb-1 flex items-center justify-end w-36 truncate mr-2"><span className="w-5 h-5 rounded-full border mr-2 shrink-0" style={{ background: getColorCode(c.productColorId!) }} />{getColorName(c.productColorId!)}: </span>
...
export function BuyRegisterForm({
  registers,
  productTypes,
  productColors,
  buyId,
}: {
  registers: BuyRegister[]; productTypes: ProductTypeWithSizes[]; productColors: ProductColor[]; buyId: string;
}) {
  const t = useClientTranslations();
  ...
  function submit() {
    setSubmitting(true);
    fetch(`/api/buy/${buyId}/buy-registers`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json'
      },
      body: JSON.stringify(formFields.filter((form) => !!form.productTypeId))
    }).then(() => location.replace(translatedPath('/app/buys')))
  }

  return (
    <div className="flex flex-col gap-4 items-start">
      <div className="max-h-[calc(var(--vh,100dvh)-300px)] overflow-auto flex flex-col gap-4 items-start w-full">
      ...
      <Button className="w-full" type="button" variant="secondary" onClick={addForm}>
